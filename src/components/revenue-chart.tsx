...
export function RevenueChart({ data }: { data: DataPoint[] }) {
  ...
  const chartData = !data || data.length === 0 ? FALLBACK_DATA : data;
        ...
        {hoverIndex !== null ? (
          <div className="text-right">
            <span className="text-[10px] text-zinc-500 font-semibold uppercase tracking-wider block">
              {chartData[hoverIndex].month} Revenue
            </span>
            <span className="text-sm font-black text-white">
              {formatINR(chartData[hoverIndex].amount)}
            </span>
          </div>
        ) : (
      ...
      <div className="relative h-48 w-full">
        <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full text-zinc-400 overflow-visible">
          ...
          {[0, 0.25, 0.5, 0.75, 1].map((ratio) => {
            ...
            return (
              <line key={ratio} x1={padding} y1={y} x2={width - padding} y2={y}
                stroke="#1f1f23" strokeWidth="1" strokeDasharray="4 4" />
            );
          })}
          ...
          {points.map((p) => {
            ...
            return (
              <g key={p.index} onMouseEnter={() => setHoverIndex(p.index)} onMouseLeave={() => setHoverIndex(null)} className="cursor-pointer">
                <circle cx={p.x} cy={p.y} r="12" fill="transparent" />
                <circle cx={p.x} cy={p.y} r={isHovered ? "5" : "3"} fill={isHovered ? "white" : "#1f1f23"} stroke="white" strokeWidth="2" className="transition-all duration-150" />
              </g>
            );
          })}
        </svg>
